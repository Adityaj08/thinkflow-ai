...
export const OptionsMenu = ({ isDarkMode, currentTheme, setCurrentTheme, setIsOptionsOpen, clearStorage }) => {
  ...
  return (
    <div className={`absolute top-12 right-0 mt-2 w-72 rounded-lg shadow-lg overflow-hidden backdrop-blur-lg
        ...
        <div className="flex items-center justify-between">
          <div>
            ...
            <p className={`text-xs ${isDarkMode ? 'text-white/60' : 'text-gray-500'}`}>
              {isAdmin ? 'Admin Plan' : isPro ? 'Pro Plan' : 'Free Plan'}
            </p>
          </div>
          {!isAdmin && !isPro && (
            <button
      ...
      <div className="p-4">
        <button
          onClick={() => {
            if (window.confirm('Are you sure you want to clear local storage? This will reset your diagram.')) {
              clearStorage();
